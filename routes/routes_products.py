...
def gerenciar_produtos():
    ...
    if request.method == 'POST':
        ...
        # --- CORREÇÃO 1: Tratamento Robusto de Campos Vazios ---
        # Garante que string vazia "" vire None (NULL no banco)
        if not codigo_barras or not codigo_barras.strip():
            codigo_barras = None
        else:
            codigo_barras = codigo_barras.strip()

        if not detalhes or not detalhes.strip():
            detalhes = None
        ...
        # --- CORREÇÃO 2: Tratamento da Medida (Vírgula para Ponto) ---
        medida_float = None
        ...
        # Verifica duplicidade MANUAL (Apenas se tiver código)
        if codigo_barras:
        ...
        try:
            ...
            db.session.rollback()
            # --- CORREÇÃO 3: Mensagens de Erro Inteligentes ---
            erro_str = str(e).lower()
            if 'unique' in erro_str and 'codigo_barras' in erro_str:
                flash(f'ERRO: O código de barras "{codigo_barras}" já está em uso.', 'error')
            elif 'unique' in erro_str and 'nome' in erro_str:
                # Se o banco impedir nomes duplicados (depende da sua modelagem)
                flash(f'ERRO: Já existe um produto com o nome "{nome_produto}".', 'error')
            else:
                flash(f'Erro de Integridade (Duplicidade): Verifique se o nome ou código já existem.', 'error')
            # --------------------------------------------------
...
def edit_produto(produto_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            # Mensagem detalhada também na edição
            if 'codigo_barras' in str(e).lower():
                 flash('Erro: Este código de barras já está sendo usado por outro produto.', 'error')
            else:
                 flash('Erro ao atualizar (possível duplicidade de nome ou código).', 'error')

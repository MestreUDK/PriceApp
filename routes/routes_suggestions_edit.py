...
from models import Supermercado, Produto, Categoria, SugestaoEdicao
...
def sugerir_edicao():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            elif tipo_item == 'categoria':
                item = db.session.get(Categoria, int(item_id))
                valor_antigo = getattr(item, campo_sugerido) # Pega 'nome'
...
def aprovar_sugestao_edicao(sugestao_id):
    ...
    try:
        ...
        # MUDANÇA: Categoria
        elif sugestao.tipo_item == 'categoria':
            item = db.session.get(Categoria, sugestao.item_id)
